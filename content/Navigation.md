...
### Mapping
Before the robot can navigate properly through the environment and ensure proper coverage, the environment must first be known. That is where mapping approaches can be helpful. Several advanced and specialised mapping approaches already exist, but in this work only one was considered: frontier-based mapping. Unlike less advanced alternatives, frontier-based mapping allows the robot to start navigating the environment and expand its map as new areas are discovered, which is well suited to partially known indoor spaces [@SlamTBX2021].
