...
\paragraph{Preprocessing}
    ...
    In this case the input will be the costmap directly. First the costmap is thresholded, which is the first output.Then using this binary costmap, the contours Figure~\ref{fig-map_contours} are extracted.
   ...
   The first implemented method, based on the work of \citep{skeleton}, \citep{skeleton2}, derives a coverage path from the morphological skeleton of the free space. See the algorithm in the appendix. 

    %fix een label voor dit aub
    
    The free space is extracted using the methods discussed in the previous section. The polygons are used and converted to a binary map as to not produce too many branches for the robot to navigate through. This map is then skeletonized to extract waypoints.
    \end{paracol}
    \newpage
\begin{paracol}
    {2}
    
    Once the waypoints are obtained, a graph is constructed and leaf nodes are extracted. In the graph leaf nodes are defined by there single connection to the rest of the skeleton, these are seen as endpoints of the path, see Figure~\ref{fig-skeleton_leaves} . Finally, starting from the closest leaf node to the robot, the full path is planned by recursively tracing the graph from the current leaf node to the nearest unvisited leaf node. 

    \begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{content/figures/navigator_results/skeleton_tree_leaves.png}
    ...
    As opposed to the Skeleton planner, this planner makes use of the binary costmap directly. Using this binary costmap, it is further discretized to a grid which is the fraction of the resolution of the original costmap, see Figure~\ref{fig-spanning_grid}. 
\switchcolumn
    \begin{figure}[!htbp]
    \centering
    ...
    \end{figure}
    A degree first search (DFS) is conducted over the free cells, from which sparse waypoints are created. A black image, of the same resolution as the original costmap is then created and white squares, with half the cell width, are placed in the center of each cell and in between each cell. Waypoints are created from the contours of this tree. These waypoints represent a circumnavigation around the DFS tree. A segment from this circumnavigation is then removed from the path and separate loops are connected to each other to make a complete path, see Figure~\ref{fig-spanning_path}. 
    \begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\linewidth]{content/figures/navigator_results/spanning_tree_path.png}
    \caption[]{Downsampled free-space grid used to construct the spanning tree graph.}
    \label{fig-spanning_path}
    \end{figure}
