...
\begin{paracol}
    ...
    \color{red}This results in a total of 16 experimental configurations.\color{black} However, some of these setups are fundamentally incompatible, such as Systematic coverage with a clean-as-you-go approach.
    
    \medskip
    \centerline{\rule{7.8cm}{0.4pt}}
    
    \subsection{From Simulation to Real-World Application}
    For the transition from simulation to real-world testing the codebase needed to be altered: The launch files were changed such that Gazebo did not run and \texttt{use\_sim\_time} parameters were defaulted to \texttt{false}. To reduce the risk of collisions, the inflation radius and the robot radius in NAV2 were increased. Another change was the migration of processing tasks from the OrangePi3B to a laptop connected to the MIRTE. Finally, all the sensors were tested and we concluded that the depth camera was not positioned correctly and thus we changed the position of the depth camera from the front of the robot to the gripper allowing its position and viewing angle to be adjusted.
