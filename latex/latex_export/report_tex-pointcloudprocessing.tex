...
First, the original point cloud is made using the depth image from the camera, as shown in Figure~\ref{og_pc}
...
Next, this point cloud is downsampled. Note that the resolution afterwards is still quite high. This is due to the fact that objects are very small, and if resolution is reduced dramatically, details of said objects will be lost, reducing bounding box accuracy. Below in Figure~\ref{downsampled} is an example.
...
In the downsampled point cloud, planes will be detected using RANSAC plane segmentation \citep{Randomsampleconsensus}. With this, large planes such as walls and tabletops will be discarded from the point cloud, as can be seen in Figure~\ref{segmented}.
...
Now, the remaining points are cross referenced with the costmap obtained during mapping. Any points that is on the costmap is excluded from the point cloud, resulting in the point cloud as depicted in Figure~\ref{ExclusivePointCloud}
...
Finally, the remaining points can be converted to clusters using \citep{DBSCAN}. This function enables the construction of bounding boxes for each cluster, which represents an object. Bounding boxes, however faint, can be made out in Figure~\ref{boundingBoxes} below.
