...
While the arm on the mirte master can be controlled in joint-space, our implementation {\textbackslash}~relies on task-space (cartesian-space) control over the end effector position. \newline
The manner in motion planners are typically implemented requires the integration of several comlpex subsystems like inverse and forward kinematic solvers, trajectory planners and path planners. To accomplish the goal of task space control over the arm, we use MoveIt 2 \citet{Coleman2014MoveIt}.
