...
This section covers the articulation of the robotic arm that sits atop the MIRTE Master. Since there is hardly any competition in the space, the market leader, MoveIt 2, will be used to move the arm. MoveIt 2 provides a well-documented API and directs the vast majority of computations regarding (inverse) kinematics of robotic arms and the trajectory planning thereafter (\href{https://moveit.picknik.ai/main/api/html/index.html}{source}).

The package is already installed on the MIRTE Master and has previously been used semi-successfully to move MIRTE's arm (\href{https://docs.mirte.org/develop/doc/tutorials/moveit.html}{source}). However, this implementation resulted in the end effector (the tip of the arm; the wrist) being positioned more than 10 cm off-target. For a machine needing to pick up objects smaller than 6 cm, this is unacceptable. Therefore, an improved version is required. To this end, the designed behaviour contains a multitude of improvements and concessions:

\begin{itemize}
\item Changes

\begin{itemize}
\item The previously missing config file moveit\_cpp.yaml which contains additional parameters (specifically for OMPL, the motion planner) was created and added.
...
\item An action server has been written which includes feedback to a potential action client, which can give a goal for the arm in two formats: either by declaring the name of a predefined joint state specified in the robot's description files, or by supplying a goal pose relative to Mirte's frame base\_link. The action server will move the arm in such a way that satisfies the goal, if possible. The wrist joint can be moved separately to allow fine adjustment of the direction of the gripper. Similarly to the arm, the gripper can be moved by inputting a named state, or by simply assigning the desired clamping angle. The gripper will move accordingly, if possible.
\end{itemize}
\end{itemize}
